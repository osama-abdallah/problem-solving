...
const employeesBonus = (arr) => {
    arr.forEach(item=>{
        if(item.workHours>8){
          item.salary=(parseInt(item.salary)+100).toString()+'$'
        }else{
          item.salary =(parseInt(item.salary)+50).toString()+'$'
...
const mostExpensive = (budget, mouseArray, keyBoardArray) => {
    let min = 0
    mouseArray.forEach(item=>{
       keyBoardArray.forEach(item2=>{
         if(item+item2> min &&item+item2<=budget){
           min= item+item2
         }
       })
    })
